...
{
    static int cnt = num;
    long long tid = tidToLl(this_thread::get_id());
...
public:
    void operator()(chrono::time_point<chrono::steady_clock> t1, int a)
    {
        ...
        LOG_INFO("[tid:%lld] hello, period_cls_func with param %d! period_cls_run_cnt = %d, time diff is %d ms\n", 
                                        tid, a, period_cls_cnt_.load(), tm_diff);
        period_cls_cnt_.fetch_add(1);
        return;
    }
    period_cls() = default;
...
void test_timer() {
    ...
    auto repeated_id = t.runRepeated(repeated_timeout_ms, repeated_cnt, test_repeated_func, t1, repeated_cnt);
    ...
    auto certion_id = t.runAt(now + std::chrono::milliseconds(run_at_ms), test_run_at_func, t1, run_at_ms);
    ...
    auto once_id = t.runAfter(run_after_ms, false, run_after_lamda);
    ...
    auto period_id = t.runAfter(run_after_ms_period, true, [t1, &p_cls](int val){ p_cls(t1, val); }, p_param);
